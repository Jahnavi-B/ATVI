...
import pyperclip,cv,playsound,requests,json 
                ...
                while(True):
                    ...
                    if(('stone' in user_ip) or ('paper' in user_ip) or ('scissor' in user_ip) or ('cutter' in user_ip) or ('rock' in user_ip)):
                        ...
                        if(user_ip=='rock'):
                            user_ip='stone'
                        break
                    #elif(('exit' in user_ip) or ('stop' in user_ip)):
                     #   human_score = 2
                    else:
...
class camera:
    def takePhoto(self):
        self.videoCaptureObject = cv.VideoCapture(0)
        self.result = True
        a=os.getcwd()
        ...
        while(self.result):
            ...
            cv.imwrite(self.ImageName,self.frame)
            self.result = False
